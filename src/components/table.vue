    ...
    >
     ...
     <q-dialog    
        ...
        <q-card class="my-card">
            <div ref="imagesFull">
                <q-img  :src="imagesByIDCom.full_picture" style="width:100%; height: 200px;"/>
            </div>
  ...
  data () {
    ...
    computed: {
        ...
        getTotalRow(){
            ...
            return this.getImages.pictures.length 
        },
        imagesByIDCom:{
            get(){
  ...
  methods: {
    ...
    onRequest (props) {
      ...
      // emulate server
        ...
        this.fetchFromServer(startRow, fetchCount, filter, sortBy, descending,page)
    ...
    fetchFromServer (startRow, count, filter, sortBy, descending, page) {
       return new Promise (async (resolve, reject) => { 
           ...
           await this.getImages(page).then(Response => {
            this.pagination.rowsNumber = Response.pageCount 
            this.pagination.rowsPerPage = Response.pictures.length  

            let data1 = filter ? Response.pictures.filter(row => row.id.includes(filter)) : Response.pictures.slice()
